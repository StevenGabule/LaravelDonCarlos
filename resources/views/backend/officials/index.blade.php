...
@section('content')
  ...
  <div class="modal" id="CreateGroupOfficialModal" tabindex="-1" role="dialog" data-backdrop="static"
       data-keyboard="false"
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form id="OfficialGroupForm" method="post" enctype="application/x-www-form-urlencoded">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal" id="closeGroupModal">Close
            </button>
  ...
  <script>
          ...
          columns: [
            ...
            },
            {
              ...
              render: (data) => {
                ...
                return `<img src="${officialsUrl}/${data}" alt="image" width="32" />`
              }
            },
            {
              data: 'name',
    ...
    $(document).on('click', '.editOfficial', function () {
      ...
      $.ajax({
        ...
        dataType: "json",
        success: ({official}) => {
          const BASE_URL = '{{ asset('storage/uploads/official_groups/thumbnail/') }}'
          const {id, name, baranggay_id, from, to, position, status, avatar} = official;
          $(".fa-goner").addClass('d-none');
          const x = $('#previewImage');
          x.addClass('d-block');
          x.attr('src', `${BASE_URL}/${avatar}`);
